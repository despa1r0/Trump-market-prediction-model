...
try:
    ...
    print("[WARN] model_trainer.py not found, make sure the file is in the same directory")
    sys.exit(1)
...
class InteractiveTrainer:
    ...
    def train_full_pipeline(self):
        ...
        try:
            ...
            print(f"   [OK] Features created: {len(self.training_columns)}")
        except Exception as e:
        ...
        print(f"\n{Fore.CYAN}[*] STEP 4: Fine Tuning Best Model...{Style.RESET_ALL}")
        try:
            # Key step that was missing earlier
            self.trainer.fine_tune_best_model()
            print(f"{Fore.GREEN}[OK] Fine tuning completed successfully!{Style.RESET_ALL}")
        except Exception as e:
            print(f"{Fore.YELLOW}[WARN] Fine tuning warning: {e}{Style.RESET_ALL}")
    ...
    def save_model_to_disk(self):
        ...
        if not self.trainer or not self.trainer.results:
            print(f"{Fore.RED}[X] No trained models! Run Option 1 first.{Style.RESET_ALL}")
            return
        ...
        while True:
            try:
                ...
                if 0 <= idx < len(models_list):
                    ...
                    print(f"\n{Fore.GREEN}[OK] Successfully saved: {filename}{Style.RESET_ALL}")
                    break
                else:
